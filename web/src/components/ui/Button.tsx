...
export const Button: React.FC<ButtonProps> = ({
  ...
  const baseClasses =
    'inline-flex items-center justify-center font-bold tracking-tight rounded-md transition-all duration-fast cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed select-none focus:outline-hidden focus:ring-2 focus:ring-interactive/40';

  // Variant mappings using semantic classes only
  const variantClasses: Record<ButtonVariant, string> = {
    primary:
      'bg-interactive hover:brightness-110 text-interactive-fg shadow-[0_0_15px_var(--color-interactive-subtle)] hover:shadow-[0_0_20px_var(--color-interactive-subtle)]',
    secondary:
      'bg-surface-card hover:bg-surface-hover text-text-primary border border-border-subtle hover:border-interactive-border',
    critical:
      'bg-status-critical-surface hover:bg-status-critical-subtle text-status-critical border border-status-critical-border hover:shadow-[0_0_15px_var(--color-status-critical-subtle)]',
    warning:
